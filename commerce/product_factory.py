...
class ProductFactory:
    ...
    @staticmethod
    def load_default_catalog(provider: CatalogProvider, max_rows=100_000) -> list["Product"]:      
        match provider:          
            case CatalogProvider.WOOCOMMERCE:
                file_path = os.path.join(CONST.ROOT_DIR, "data/woocommerce/product_catalog.csv")
                return ProductFactory.tryload_catalog_woo(file_path, max_rows)
            case CatalogProvider.SHOPIFY:
                file_path = os.path.join(CONST.ROOT_DIR, "data/shopify/shopify_products.csv")
                return ShopifyConverter.tryload_catalog_shopify(file_path, max_rows)
            case CatalogProvider.AMAZON:
                file_path = os.path.join(CONST.ROOT_DIR, "data/amazon/fashion/amazon_fashion_sample_1000.json")
    ...
    @staticmethod
    def tryload_catalog_woo(file_path: str, max_rows=100_000) -> list:
        """
    ...
    def tryload_catalog_to_json(provider: CatalogProvider, file_path: str, max_rows=100_000) -> str:
        ...
        match provider:
            case CatalogProvider.WOOCOMMERCE:
                thing = ProductFactory.tryload_catalog_woo(file_path, max_rows)
            ...
            case CatalogProvider.WALMART:
                thing = WalmartConverter.tryload_catalog_csv(file_path, max_rows)        
                return json.dumps(thing, indent=2)
            case _:
               raise ValueError(f"Invalid provider: {provider}")
...
class WalmartConverter(BaseConverter):    
    ...
    @staticmethod
    def tryload_catalog_csv(file_path: str, max_rows=100_000) -> list:
        """
